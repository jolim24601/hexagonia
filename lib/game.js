...
Hexscape.Game.prototype = {
  ...
  loadLevel: function (level) {
    var playData = Hexscape.PLAYER_DATA[level - 1];
  ...
  create: function () {
    ...
    if (this._levelNumber === 1
        && Hexscape.PLAYER_DATA[this._levelNumber - 1].attempts === 1) {
  ...
  createBoard: function (params) {
    ...
    for (var row = 0; row < Hexscape.GRID_SIZE_Y; row++) {
        ...
        // pos logic only for 'playable' hexagons
    ...
    this.loadSpritePhysics();
  },
  loadSpritePhysics: function () {
  ...
  addBounce: function (body, _pointer) {
    body.velocity.x *= this.bounceFactor;
    body.velocity.y *= this.bounceFactor;
  },
  teleport: function (teleport) {
    this.ball.body.x = teleport.xPos;
    this.ball.body.y = teleport.yPos;
    
    this.teleported = true;

    this.shutDownTele();
  },
  shutDownTele: function () {
    this.teleports.forEach(function (tele) {
      tele.sprite.destroy();
    });
  },
  ...
  createOptions: function () {
    ...
    var attempts = Hexscape.PLAYER_DATA[this._levelNumber - 1].attempts;
    ...
    if (this.roundWon) {
      Hexscape.PLAYER_DATA[this._levelNumber - 1].attempts = 0;
    }
  },
  ...
  gameWon: function () {
    var playData = Hexscape.PLAYER_DATA[this._levelNumber - 1];
    ...
    if (this._levelNumber < Hexscape.PLAYER_DATA.length) {
      if (Hexscape.PLAYER_DATA[this._levelNumber].stars < 0) {
        Hexscape.PLAYER_DATA[this._levelNumber].stars = 0;
      }
    }
  ...
  savePlayerData: function () {
    window.localStorage.setItem('hexscape_progress', JSON.stringify(Hexscape.PLAYER_DATA));
  }
}
