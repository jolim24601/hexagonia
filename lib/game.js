...
var HexagonGroup, MenuGroup, OptionsGroup, ResetGroup, MuteGroup, LevelGroup;
...
Hexscape.Game.prototype = {
  preload: function () {
    ...
    clickFx = this.add.audio('reverse');
  },
  ...
  createResetButton: function () {
    ...
    ResetGroup.x = 125; ResetGroup.y = 0;

    resetButton.inputEnabled = true;
    resetButton.events.onInputDown.add(this.restartLevel, this);
    this.holdIcons['ResetGroup'] = ResetGroup;
  },
  restartLevel: function () {
    var tween = this.add.tween(this.holdIcons.ResetGroup.scale)
  ...
  createMenu: function () {
    ...
    this.holdIcons['MenuGroup'] = MenuGroup;
  },
  ...
  createBoard: function (params) {
    ...
    for (var row = 0; row < GRID_SIZE_Y; row++) {
        ...
        this.holdIcons['HexagonGroup'] = HexagonGroup;
  		}
  	}
  ...
  pauseGame: function () {
    var tween = this.add.tween(this.holdIcons.MenuGroup.scale)
    .to({ x: 0.9, y: 0.9 }, 100, Phaser.Easing.Linear.None)
    .to({ x: 1.0, y: 1.0 }, 100, Phaser.Easing.Linear.None)
    .start();
    tween.onComplete.add(function() {
  ...
  showMenu: function () {
    ...
    OptionsGroup.x = this.world.centerX; OptionsGroup.y = this.world.centerY;
    ...
    var levelText = this.add.text(50, -50, 'Level: ' + this._levelNumber, style);
    var attemptsText = this.add.text(50, -25, 'Attempts: ' + attempts, style);
    levelText.alpha = 0.8; attemptsText.alpha = 0.8;
    OptionsGroup.add(levelText); OptionsGroup.add(attemptsText);

    var LevelGroup = this.add.group();
    var levelSelectGon = this.add.sprite(0, 0, 'display');
    LevelGroup.add(levelSelectGon);
    var levelGridIcon = this.add.sprite(28, 28, 'grid');
    levelGridIcon.scale.setTo(1/3);
    levelSelectGon.scale.setTo(1/2);
    LevelGroup.add(levelGridIcon);
    OptionsGroup.add(LevelGroup);
    LevelGroup.x = -150; LevelGroup.y = -75;
    levelSelectGon.inputEnabled = true;
    levelSelectGon.events.onInputDown.add(this.goBack, this);
    this.holdIcons['LevelGroup'] = LevelGroup;
  },
  goBack: function () {
    PLAYER_DATA[this._levelNumber - 1].attempts += 1;
    var tween = this.add.tween(this.holdIcons.LevelGroup.scale)
    .to({ x: 0.9, y: 0.9 }, 100, Phaser.Easing.Linear.None)
    .to({ x: 1.0, y: 1.0 }, 100, Phaser.Easing.Linear.None)
    .start();
    tween.onComplete.add(function() {
  ...
  gameWon: function () {
    ...
    this.state.start('GameOver', true, false, { level: this._levelNumber, roundWon: true });
  },
  gameLost: function (ball) {
    if (this.roundWon) { return; }
    PLAYER_DATA[this._levelNumber - 1].attempts += 1;
    this.state.start('GameOver', true, false, { level: this._levelNumber, roundWon: false });
  },
}
