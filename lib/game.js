...
Hexscape.Game.prototype = {
  ...
  update: function () {
    if (!this.teleported
        && this.teleportFrom
        && this.checkOverlap(this.teleportFrom, this.ball)) {
  ...
  checkOverlap: function (teleGon, ball) {
    if (ball.x > teleGon.x - teleGon.width/2
        && ball.x < teleGon.x + teleGon.width/2
        && ball.y > teleGon.y) {
          return true;
        }
  ...
  teleport: function () {
    if (!this.teleported) {
      this.ball.reset(this.teleCoords[0], this.teleCoords[1]);
  ...
  createSpecialHex: function (row, col, type) {
    ...
    if (type == 'block') {
      ...
      var teleportPos = Hexscape.LEVEL_DATA[this._levelNumber - 1].telePos[1];
      if (teleportPos[0] == row && teleportPos[1] == col) {
        this.teleCoords = [xPos, yPos];
      } else {
        this.teleportFrom = special;
      }
    }
