...
Hexscape.LevelSelect.prototype = {
  ...
  create: function () {
    ...
    var title = this.add.bitmapText(0, 60, 'font72', 'Select a Level', 72);
  ...
  initProgressData: function () {
    if (!Hexscape.PLAYER_DATA) {
      var data = window.localStorage.getItem('hexscape_progress');
      Hexscape.PLAYER_DATA = JSON.parse(data) || [];
    }
  },
  createLevelIcons: function () {
    ...
    for (var y=0; y < 3; y++) {
      for (var x=0; x < 4; x++) {
        ...
        Hexscape.PLAYER_DATA[level - 1] = Hexscape.PLAYER_DATA[level - 1] || {};
        if (typeof Hexscape.PLAYER_DATA[level - 1].stars !== 'number') {
          if (level == 1) {
            Hexscape.PLAYER_DATA[level - 1].stars = 0;
          } else {
            Hexscape.PLAYER_DATA[level - 1].stars = -1;
          }
        }
  ...
  createLevelIcon: function (xpos, ypos, level, isLocked, stars) {
    ...
    if (isLocked === false) {
      text = this.add.text(60, 70, '' + level, style);
      text.align = 'center';
      text.anchor.set(0.5);
  ...
  onSpriteDown: function (sprite, pointer) {
    ...
    if (Hexscape.PLAYER_DATA[level - 1].stars < 0) {
